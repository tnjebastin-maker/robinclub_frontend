...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login({ email: data.email, name: data.name, role: data.role, phone: data.phone }, data.token);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <img src={logo} alt="Robin Club" className="w-16 h-16 rounded-full object-cover mx-auto mb-4 ring-2 ring-blue-500" />
          <h1 className="text-2xl font-bold text-gray-900">Sign in to Robin Club</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          ...
          <button onClick={() => googleLogin()}
            className="w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-50 text-gray-700 py-2.5 rounded-lg font-medium transition">
